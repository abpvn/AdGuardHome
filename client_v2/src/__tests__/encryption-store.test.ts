...
describe('setTlsConfig', () => {
    ...
    it('empties a server-name list and port the backend leaves out of the response', async () => {
        ...
        expect(encryptionState.server_names).toEqual(['dns.example.com']);
        ...
        await setTlsConfig({ server_names: [], port_https: 0 });

        expect(encryptionState.server_names).toEqual([]);
...
describe('stale validation status', () => {
    ...
    it('validateTlsConfig clears the warning a corrected response omits', async () => {
        ...
        await validateTlsConfig({ enabled: true, server_names: ['adguard.example.com'] });
        ...
        await validateTlsConfig({ enabled: true, server_names: ['wrong.example.com'] });
    ...
    it('setTlsConfig clears the warning a corrected save omits', async () => {
        ...
        await setTlsConfig({ server_names: ['adguard.example.com'] });

        expect(encryptionState.warning_validation).toBe(SERVER_NAME_MISMATCH_WARNING);

        mocks.tlsConfigure.mockResolvedValue(RESPONSE_AFTER_FIX);
        await setTlsConfig({ server_names: ['wrong.example.com'] });
