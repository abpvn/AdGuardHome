...
/**
 * Settings the backend marshals with `omitempty`: a cleared server-name list,
 * a port that is turned off, and a resolved validation warning with the
 * certificate metadata behind it all come back as missing keys rather than
 * empty values.  The store merges every response into its state, so the
 * absence has to be spelled out — otherwise a cleared value would silently
 * keep the previous one until the page is reloaded and the state is built
 * from scratch.
 *
