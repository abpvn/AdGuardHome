...
func confFromTLSSettings(s *tlsConfigSettings) (conf *aghtls.ExtendedTLSConfig) {
	conf = &aghtls.ExtendedTLSConfig{
		...
		ServerNames:          slices.Clone(s.ServerNames),
...
func confToTLSSettings(conf *aghtls.ExtendedTLSConfig) (s tlsConfigSettings) {
	return tlsConfigSettings{
		...
		ServerNames:          slices.Clone(conf.ServerNames),
...
func validateCertificates(
	...
	if len(certChain) > 0 {
		var ok bool
		ok, err = validateCertificate(ctx, logger, tlsConfProvider.RootCAs(), status, certChain, serverNames)
		if !ok {
