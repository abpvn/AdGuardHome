...
func TestValidateCertificates(t *testing.T) {
	...
	t.Run("bad_certificate", func(t *testing.T) {
		...
		err = m.validateCertificates(ctx, status, []byte("bad cert"), nil, []string{""})
	...
	t.Run("bad_private_key", func(t *testing.T) {
		status := &tlsConfigStatus{}
		err = m.validateCertificates(ctx, status, nil, []byte("bad priv key"), []string{""})
	...
	t.Run("valid", func(t *testing.T) {
		status := &tlsConfigStatus{}
		err = m.validateCertificates(ctx, status, testCertChainData, testPrivateKeyData, []string{""})
		assert.Error(t, err)
